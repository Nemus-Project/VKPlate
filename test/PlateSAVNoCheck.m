...
for m = 1 : Nmodes
   ...
   pext(:,m)=1e-1*Fext*mdShapes(floor(end/4),floor(end/4),m);
...
xivec=ones(Nmodes,1)*1e-1;
xi=diag(xivec);
DSM=(0.5*k*xi*Dw1+Id)^-1;
Ddamp=diag(exp(xivec))*(diag(exp(sqrt(xivec.^2-k*Om.^2)))+diag(exp(-sqrt(xivec.^2-k*Om.^2))));
Dqnmex=diag(exp(-2*xivec*k));
...
for n = 2 : Ts
    ...
    %qs=Dexp*(Ddiag*q0+Dqnm*qm-g*(k^2)*psim1+k^2*pext(:,n));

    qs=Dexp2*(D0sv*q0+Dqnm2*qm-g*(k^2)*psim1+k^2*pext(:,n)); %non exact scheme

    %qs=Dexp*(Ddamp*q0+(Dqnmex+0.25*(k^2)*g*g')*qm-g*(k^2)*psim1+k^2*pext(:,n));
...
ncheck=400;
figure
plot(t,En-En(ncheck),t,V0-V0(ncheck),t,((PSI0.^2)-(PSI0(ncheck)^2))/2)
figure
EN=En+V0+(PSI0.^2)/2;
scatter(t,(EN-EN(ncheck))/EN(ncheck),'.')
xlim([0.02 T])
%%
% Fs=44100;
