...
lbb=10;
back=zeros(lbb,1) % backbone curve vector
qinibb=linspace(10e-8,10e-4,lbb);

%% Initialisation

%qm=flipud(sort(1e-6*rand(Nmodes,1)));           %random excitation
for bb=1:lbb
qm(1)=qinibb(bb);                                     % Initialisation of the displacement at n-1
...
for n = 2 : Ts
    ...
    %g=enerr*(1/sqrt(2*U+ep))*dUdq-l0*dr(n-1); %g with drift control
    ...
    %psi0=psim1+0.5*g'*(qs-qm)-l0*dr(n-1); % Psi control drift
